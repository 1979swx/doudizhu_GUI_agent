...
DATA_DIR="${DATA_DIR:-data_synthesis/doudizhu_end_to_end_sft}"
TRAIN_FILE="${TRAIN_FILE:-${DATA_DIR}/train.parquet}"
VAL_FILE="${VAL_FILE:-${DATA_DIR}/val.parquet}"

MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3.5-4B}"
OUTPUT_DIR="${OUTPUT_DIR:-checkpoints/sft/qwen3_5_4B_doudizhu_end_to_end}"
PROJECT_NAME="${PROJECT_NAME:-verl_agent_sft}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen3_5_doudizhu_end_to_end_sft}"
...
require_file "${TRAIN_FILE}" "Generate end-to-end data first with data_synthesis/doudizhu_end_to_end_sft.py."

VAL_ARGS=()
if [[ -f "${VAL_FILE}" ]]; then
  VAL_ARGS=(--val-file "${VAL_FILE}")
else
  echo "No validation parquet found at ${VAL_FILE}; running train-only SFT." >&2
fi
