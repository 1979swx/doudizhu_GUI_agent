...
OUTPUT_DIR="${OUTPUT_DIR:-checkpoints/sft/qwen3_5_4B_doudizhu_grounding_qa_mix}"
PROJECT_NAME="${PROJECT_NAME:-verl_agent_sft}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen3_5_doudizhu_grounding_qa_mix_sft}"
...
require_file "${GROUNDING_VAL_FILE}" "Generate grounding data first with data_synthesis/doudizhu_grounding_sft.py."
require_file "${QA_TRAIN_FILE}" "Generate QA data first with data_synthesis/doudizhu_qa_sft.py."
require_file "${QA_VAL_FILE}" "Generate QA data first with data_synthesis/doudizhu_qa_sft.py."

TRAIN_FILES=("${GROUNDING_TRAIN_FILE}" "${QA_TRAIN_FILE}")
VAL_FILES=("${GROUNDING_VAL_FILE}" "${QA_VAL_FILE}")
