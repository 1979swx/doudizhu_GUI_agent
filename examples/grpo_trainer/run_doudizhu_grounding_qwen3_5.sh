...
PROJECT_NAME="${PROJECT_NAME:-verl_agent_doudizhu_grounding}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-grpo_qwen3_5_4b_doudizhu_grounding_zh}"
NUM_GPUS="${NUM_GPUS:-2}"
...
python3 -m verl.trainer.main_ppo \
    ...
    env.env_name=doudizhu_grounding \
    env.seed=0 \
    env.max_steps="${max_env_steps}" \
    env.rollout.n="${group_size}" \
    env.resources_per_worker.num_cpus="${num_cpus_per_env_worker}" \
    env.doudizhu.use_ray=False \
    env.doudizhu.language=zh \
    env.doudizhu.chinese_mode=True \
    env.doudizhu_grounding.use_ray=False \
    env.doudizhu_grounding.teacher_policy=rule_v1 \
