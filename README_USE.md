...
4. 环境把点击还原为斗地主动作，记录投影合法性、点击命中、规则合法性、手牌推进和胜负结果。
...
| 阶段 | 评测单位 | 胜率 | 投影合法率 | 规则合法动作率 | fallback 率 | 模型自主出牌推进 |
|---|---:|---:|---:|---:|---:|---:|
| Qwen3.5-4B baseline | 128 局 | 0.00% | 66.95% | 5.28% | 94.72% | 1.21% |
| Qwen3.5-9B baseline | 128 局 | 0.00% | 59.49% | 2.85% | 97.15% | 1.25% |
| Kimi K2.6 raw teacher | 485 局离线 raw | 10.52% | 91.87% | 63.32% | 36.65% | 48.91% |
| Grounding-only SFT 400 | 256 局 | 0.00% | 93.81% | 22.76% | 77.24% | 8.54% |
| End-to-end mix SFT 600 | 256 局 | 4.69% | 94.85% | 68.30% | 31.70% | 42.23% |
| GRPO step 60 | 256 局 | 17.58% | 99.21% | 80.42% | 19.58% | 69.77% |
| GRPO step 105 | 256 局 | 16.41% | 93.25% | 68.04% | 31.96% | 56.19% |
...
| `tests/environments/test_doudizhu.py` | 环境、投影、prompt 和 grounding group 行为测试。 |
...
完整游戏环境在非法动作时会执行 fallback，以保证 episode 可继续，但 fallback 会被指标记录下来。当前核心目标之一就是降低 fallback 依赖。
