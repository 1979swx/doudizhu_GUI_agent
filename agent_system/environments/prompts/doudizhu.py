...
输出格式
针对当前牌面进行一步步的思维链策略思考，包括盘点手牌、分析局势、组合推演等；将你的行动表述为 JSON 风格的列表，包含一个或多个 [x, y] 坐标对(x 和 y 必须是1到1000范围内的数字)；输出对其它玩家说的聊天内容；为下回合生成一份简短的记忆，包括对手可能持有的高价值牌与危险牌型、未来战术规划等。
