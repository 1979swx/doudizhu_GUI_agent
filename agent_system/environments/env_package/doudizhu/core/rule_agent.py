...
from .utils import CARD_TYPE, INDEX


class DouDizhuRuleAgentV1:
    """A lightweight heuristic Dou Dizhu bot from RLCard."""
    ...
    def step(self, state):
        ...
        if len(trace) == 0 or (len(trace) >= 3 and trace[-1][1] == "pass" and trace[-2][1] == "pass"):
            ...
            if target_player != landlord and state["self"] != landlord:
                return "pass"
            return self.rng.choice(list(state["actions"]))

    def eval_step(self, state):
        return self.step(state), []

    def combine_cards(self, hand):
        comb = {"rocket": [], "bomb": [], "trio": [], "trio_chain": [], "solo_chain": [], "pair_chain": [], "pair": [], "solo": []}
        if hand[-2:] == "BR":
