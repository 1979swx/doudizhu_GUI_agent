...
def build_prompt(mode: str, command: str, language: str) -> str:
    if mode == "plan":
        template = PLAN_TEMPLATE_ZH if language == "zh" else PLAN_TEMPLATE
    elif mode == "no_plan":
...
def strict_plan_projection(text_actions: list[str], max_clicks: int) -> tuple[list[dict[str, Any]], list[int]]:
    structured_actions: list[dict[str, Any]] = []
    valids: list[int] = []
    for response in text_actions:
        if not isinstance(response, str):
...
def project_one(mode: str, response: str, max_clicks: int) -> dict[str, Any]:
    if mode == "plan":
        actions, valids = strict_plan_projection([response], max_clicks=max_clicks)
    elif mode == "no_plan":
